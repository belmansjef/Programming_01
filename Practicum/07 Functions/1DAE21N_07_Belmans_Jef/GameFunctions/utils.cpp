...
namespace utils
{
	...
	{
		FillEllipse(circle.center.x, circle.center.y, circle.radius, circle.radius);
	}
	...
	{
		return float((rand() % (int((max - min) * 100) + 1) + int(min * 100.0f)) / 100.0f);
	}
	...
	void GenerateRandomCirclef(Circlef& circle, float windowWidth, float windowHeight, float border, float minRadius, float maxRadius)
	{
		circle = Circlef(Point2f(GenerateRandomNumber(border, windowWidth - border), GenerateRandomNumber(border, windowHeight - border)), GenerateRandomNumber(minRadius, maxRadius));
	}

	void GenerateRandomRectf(Rectf& rect, float windowWidth, float windowHeight, float border, float minWidth, float minHeight, float size)
	{
		...
		const float width{ GenerateRandomNumber(minWidth, minWidth + size) };
		const float height{ GenerateRandomNumber(minHeight, minHeight + size) };

		rect = Rectf(left, bottom, width, height);
	}
